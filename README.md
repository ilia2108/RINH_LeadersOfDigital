...
4. При "Error 403" необходимо скачать [модель]('https://storage.b-labs.pro/models/fasttext-social-network-model.bin') и перенести ее в папку, скаченную ранее,
...
7. Дополнительную информацию по работе алгоритма можно также найти в ```server.py```
## Постановка задачи
